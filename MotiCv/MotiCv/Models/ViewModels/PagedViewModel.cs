           ...
           where T : class
    {
        ...
        public HtmlString GetPagenation(IUrlHelper urlHelper, string action, string area = "", string paginateFunction = "")
        {
            ...
            for (int i = (min < 1 ? 1 : min); i <= max; i++)
            {
                if (i == CurrentIndex)
                {
                    builder.Append($"<li class='active pagination'><a>{i}</a></li>");
                    continue;
                }
                ...
                builder.Append($"<li class='pagination'><a href='{link}'>{i}</a></li>");
            ...
            if (CurrentIndex < MaxPageIndex)
            {
                ...
                builder.Append($@"<li class='next pagination'>
                                <a href='{link}'><i class='fas fa-chevron-right'></i></a>
                                </li>");
            }
            else
            {
                builder.Append("<li class='next disabled pagination'><a><i class='fas fa-chevron-right'></i></a></li>");
            }
